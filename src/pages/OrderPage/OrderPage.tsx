...
export default function OrderPage() {
  ...
  const [amountValue, setAmountValue] = useState<number | null>(null);  // Изменено на null
  ...
  const [weightValue, setWeightValue] = useState<number | null>(null);  // Изменено на null
  ...
  const calculateDistanceValue = useCallback(async () => {
    ...
    if (distanceData !== null) {
      const value = Math.floor(distanceData.distance / 1000);
  ...
  const fetchOrderDetailsCallback = useCallback(async () => {
    if (onLoadingCityValue && onLoadingValue && onUnloadingCityValue && onUnloadingValue) {
      try {
        ...
        const orderDetails = await fetchOrderDetails(
          cargoValue, weightValue!, amountValue!, date,
          onLoadingCityValue, onLoadingValue, onLoadingPhoneValue,
          onUnloadingCityValue, onUnloadingValue, onUnloadingPhoneValue,
          isChecked // Передаем значение булевого типа напрямую
        );
    ...
    cargoValue, weightValue, amountValue, date,
    onLoadingCityValue, onLoadingValue, onLoadingPhoneValue,
    onUnloadingCityValue, onUnloadingValue, onUnloadingPhoneValue, isChecked
  ]);
  ...
  useEffect(() => {
    const checkInputs = () => {
      return cargoValue && weightValue !== null && selectedUnit && amountValue !== null && date.length === 10 && time.length === 5 &&
        ...
        distanceValue !== null; // Check for distanceValue
    };
  ...
  const handleOrderSubmit = async () => {
    if (!isButtonDisabled) {
      ...
      if (orderDetails) {
        ...
        // Here you can add further actions like redirecting to a confirmation page, showing a success message, etc.
      }
    }
  };
